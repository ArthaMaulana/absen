...
function onFormSubmit(e) {
  try {
    ...
    const noTelpCol = headers.indexOf('No. Telpon');
    const instansiCol = headers.indexOf('Instansi/Umum');
    const asalDaerahCol = headers.indexOf('Asal Daerah');
    const buktiInstagramCol = headers.indexOf('Bukti Mengikuti Instagram English Club');
    const qrIdCol = headers.indexOf('QR ID');
    const qrImageCol = headers.indexOf('QR Image');
    
    // Validasi kolom yang diperlukan
    if (emailCol === -1 || namaCol === -1 || noTelpCol === -1 || instansiCol === -1 || 
        asalDaerahCol === -1 || qrIdCol === -1 || qrImageCol === -1) {
    ...
    const qrData = {
      ...
      instansi: data[instansiCol],
      asalDaerah: data[asalDaerahCol],
...
function getVisitorData(qrData) {
  try {
    ...
    const columnIndexes = {
      ...
      noTelp: headers.indexOf('No. Telpon'),
      instansi: headers.indexOf('Instansi/Umum'),
      asalDaerah: headers.indexOf('Asal Daerah'),
    ...
    // Validasi kolom yang diperlukan
    const requiredColumns = ['email', 'nama', 'noTelp', 'instansi', 'asalDaerah', 'qrId'];
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (storedId === searchId) {
        ...
        const visitorData = {
          ...
          instansi: row[columnIndexes.instansi],
          asalDaerah: row[columnIndexes.asalDaerah],
          qrId: storedId
        };
    ...
    if (qrData.nama && qrData.email && qrData.noTelp && qrData.instansi && qrData.asalDaerah) {
      return {
        success: true,
        data: {
          ...qrData,
